...
jQueryInclude(function () {
  ...
  var botStarter = setInterval(function () {
    ...
    if (jQ("._2vbYK").filter(":visible").length > 0) {
      window.WappBot = {
        configWappBot: {
          ...
          messageInitial: (chatId) => {
            return {
              text: WappBot.configWappBot.covidQueries.msgWelcome.replace(/{{Version}}/g, Version),
          ...
          covidQueries: {
            "Version": "{{Version}}",
            "msgWelcome": "Welcome to WhatsApp Bot Version: {{Version}}\n\nPlease enter 10 digit Mobile Number between (9876543200 - 9876543299)",
            ...
            "qryApp": {
              ask: "Please enter 10 digit Mobile Number between (9876543200 - 9876543299)\n",
              options: {
                "Mobile": "qryName",
              },
              scores: {
            ...
            "qryFinished": {
              ask: "Do you want to search another?\n"
                + "1 ➙ Yes\n"
                + "2 ➙ No",
              options: {
                "1": "qryName",
       ...
       * 
       * Source: https://github.com/mukulhase/WebWhatsapp-Wrapper/blob/master/webwhatsapi/js/wapi.js
       */
      if (!window.Store) {
        (function () {
      ...
      */
      ...
      window.WappBot.sendByLocalSetting = (newMessage, chatId) => {
        ...
        if (!messageIncludeKey) { // If incorrect option or just started chatting
          ...
          console.log("Processing: " + sessionStorage.getItem(chatId + "-currQryKey"));
          if (sessionStorage.getItem(chatId + "-currQryKey") == "qryFinished") {
            window.WAPI.sendMessage(chatId, "Searching... " + newMessage);
            jQ.ajax({
              type: 'GET',
              url: BaseURL + 'cards',
              dataType: 'json',
              ...
              try {
                let obj = data.filter(item => item.mobile === newMessage);
                window.WAPI.sendMessage(chatId, "Card Holder: " + obj[0].owner);
                localStorage.setItem('Data', JSON.stringify(data));
              }
              catch (e) {
                window.WAPI.sendMessage(chatId, "Error: " + e);
                localStorage.setItem(chatId + '-Error:', e);
              }
            }).fail(function (FailMsg) {
              window.WAPI.sendMessage(chatId, "Error: " + FailMsg.statusText);
              localStorage.setItem(chatId + '-Fail:', FailMsg.statusText);
            });
          }
      ...
      jQ("._2bBPp").before('<div id="covid-tag" style="padding-bottom:25px;font-size:15px;"><div style="padding-bottom:15px;font-size:25px;color:#1B9A59">WhatsApp Bot</div><p>' + Version + '</p></div>');
      clearInterval(botStarter);

      var newMessageCallback = (message) => {
        console.log("NewMessage", message);
        if (message.length > 0 ) {
          message.forEach((newMessage) => {
            window.WappBot.sendByLocalSetting(newMessage.body, newMessage.chatId._serialized);
          });
        }
      };

      window.WAPI.waitNewMessages(false, newMessageCallback);
